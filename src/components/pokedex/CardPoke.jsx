...
const CardPoke = ({ url }) => {
  ...
  //Enviar a otra pagina al momento de hacer el evento de onClick
      ...
      <ul className='card-poke__stats-container'>
        {
          pokemon?.stats.map(stat =>(
            <li key={stat.stat.name} className='card-poke__stat'>
              ...
              <span className={`card-poke__stat-numer letter-${pokemon?.types[0].type.name}`}>{stat.base_stat}</span>
            </li>
          ))
        }
      </ul>
